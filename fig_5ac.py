...
rho = int(0.4*N) #Radius of absorption in discrete uncoupled model

#Diffusion constants for both axes
Dx = 10**(-4)
Dy = 10**(-4)

#Initial state for both axes
startposx = int(0.8*N)
startposy = int(0.8*N)

#Drift constants for both axes
vx = -8*(10**(-5))
vy = -8*(10**(-5))
...
for taul in taulvals:
    ...
    rx = (1/taul[0])*(1/365) #Inverse of therapy switching rate
    ry = rx
    
    params = [N, rho, rx, ry, vx, vy, Dx, Dy]
    #Simulate the runs for the uncoupled discrete model with limited switching
    for _ in range(runs):
        rdtvals.append(uncoupled.disc_sim_RDT(params, startposx, startposy, res_type = "limited", lim = taul[1]))
...
for tauc in taucvals:
    ...
    params = [N, rho, rx, ry, vx, vy, Dx, Dy]
    for _ in range(runs):
        rdtvals.append(uncoupled.disc_sim_RDT(params, startposx, startposy, res_type = "costed", cost = tauc[1]))
